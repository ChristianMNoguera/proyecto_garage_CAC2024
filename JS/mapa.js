...
const crearMarcador = ()=> {
    for (i in parking) {
        ...
        let customPopup = `<h2 class="popup-titulo">${parking[i].nombre} </h2><br> <h3 class="popup-dir">dirección: <br><hr> ${parking[i].direccion} </h3><br> <p class="popup-lugares">Lugares Disponibles: ${parking[i].lugares}</p>`;

        let marcador_ = L.marker([parking[i].lat, parking[i].lng], { icon: iconMarker})
            .addTo(mapaActual);
        marcador_
